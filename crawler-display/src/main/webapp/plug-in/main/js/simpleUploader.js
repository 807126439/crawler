...
function createSimpleUploader(elem,option,displayOption) {
  ...
  uploader.on('fileQueued', function(file) {
    var $item=$('<div class="file-item '+ file.id+'" >\
                  <img class="file-item-icon" src="images/fileicon/icon_large_'+matchType(file.ext)+'.png">\
                  <a href="javascript:void(0)" class="file-item-right remove"><i class="icon_close"></i></a>\
                  <div class="file-item-name" title="'+file.name+'">' + file.name + '</div>\
                  <div class="file-item-size">' + formatSize(file.size)+ '(<span class="state">等待上传...</span>)</div>\
                  <div class="file-item-contrls">\
                    <a href="javascript:void(0)" class="download"><i class="icon_download"></i><span class="text">下载</span></a>\
                    <a href="javascript:void(0)" class="preview"><i class="icon_view"></i><span class="text">预览</span></a>\
                    <a href="javascript:void(0)" class="del"><i class="icon_close"></i><span class="text">删除</span></a>\
                  </div>\
                </div>');
    ...
    uploader.makeThumb(file, function(error, src) {
      ...
      $item.find('img')[0].src=src;
    }, displayOption.thumbWidth||47,displayOption.thumbHeight||47);
    ...
    if (!displayOption.del)
      ...
      $item.find('.del').on('click',displayOption.del)
  ...
  uploader.on('uploadError', function(file) {
    ...
    if ($wrap.find('.' + file.id).find('.retry').length === 0) {
...
function matchType(ext) {
  ext=(ext+"").toLowerCase()
  switch (ext) {
    case 'png': case 'gif': case 'jpg': case 'jpeg': case 'bmp': case 'tif': case 'pcd': case 'qti': case 'qtf': case 'tiff':
    return 'img';
    ...
    case 'rar': case 'zip':
    return 'rar';
    case '3gp': case 'mp4': case 'avi': case 'mpv2': case 'mpg': case 'mpe': case 'mpeg': case 'mpeg2': case 'mpeg4': case 'rmvb': case 'rm': case 'asf': case 'divx': case 'wmv': case 'mkv': case 'vob': case 'flv': case 'ogm': case 'qt': case 'dvd':
    return 'video';
    case 'mp3': case 'cd': case 'ogg': case 'wma': case 'wav': case 'mp3pro': case 'rm': case 'real': case 'ape': case 'module': case 'midi': case 'vqf': case 'aac': case 'mac': case 'au': case 'cda':
    return 'mp3';
    default:
      return 'other';
  }
