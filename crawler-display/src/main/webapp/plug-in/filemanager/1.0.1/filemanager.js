...
//事件 fm.checked 选中文件时触发，例：$('#filemanager').on('fm.checked',function(event,checkedItem,checkedItems){}),checkedItem是当前被选中项的数据，checkedItems为所有被选中项的数据
//事件 fm.unchecked 取消选中后,参考fm.checked
//事件 fm.init 初始化完成后
//事件 fm.draw 文件视图渲染完成后，参数为所有被选中项的数据
//事件 fm.click 文件图标被点击后，参数1是当前被点击项的数据，参数2为所有被选中项的数据
function createFileManager(elem, option, uploaderOption) {
  ...
  var $wrap = $(elem),
    $toolbar=$('<div class="filemanager-toolbar">\
                  ...
                  <span class="filemanager-filter">\
                    <input type="text" class="filemanager-search" placeholder="搜索关键字..."/>\
    ...
    $uploaderBg=$uploaderContainer.find('.uploader-bg'),
  ...
  if(option.sortMenu&&option.sortMenu[0])
    $('<li data-val="gmtCreate" class="sort-gmtCreate">上传时间</li>').appendTo($sortMenu);
  ...
  if(option.sortMenu&&option.sortMenu[2])
    $('<li data-val="fileType" class="sort-fileType">类型</li>').appendTo($sortMenu);
  ...
  $uploaderBg.on('click',closeUploader);
  ...
  $view.on('click','.view-item-icon',function() {
    // 监听
    var $item=$(this).closest('.view-item');
    $item.trigger('fm.click',[fileManager.fileDataList[$item.index()],fileManager.fileDataList])
  });
...
//        };
        ...
        $.each(data.list, function(i, item) {
          var ext=item.fileName.match(/\.(\w+)$/)[1];
          var type=matchType(ext);
          var iconClass="fileicon-"+type;
          var bgstyle='';
          if (item.pattern) {
            bgstyle="style='background-image:url("+item.pattern+");background-size:cover;'";
          }
          str += "<div class='view-item' data-itemid='"+item.id+" 'data-type='"+type+"' data-src='"+(typeof(item.bigPattern)!=='undefined'?item.bigPattern:"") + "'>\
                    <div class='view-item-icon "+iconClass+"' "+bgstyle+"></div>\
                    <div class='view-item-name'>"+item.fileName+"</div>\
                    <div class='view-item-addtime'>"+(item.gmtCreate||'')+"</div>\
        ...
        $view.trigger('fm.draw',fileManager.fileDataList);
      }
    });
  }
  ...
  function fileDownload() {
    ...
    window.location.href = option.downloadBtn.url+l;
  ...
  $view.on('click','.view-item-checkbox',function() {
    ...
    afterCheck();
    var ev=$item.hasClass('item-checked')?'fm.checked':'fm.unchecked'
    $(this).trigger(ev,[fileManager.fileDataList[$item.index()],fileManager.fileDataList])
  });
  $checkAll.on('click',function() {
    $(this).toggleClass('checked');
    if ($(this).hasClass('checked')) {
      var $targetItems=$view.find('.view-item').not('.item-checked');
      var ev='fm.checked';
      $targetItems.addClass('item-checked');
    }else{
      var $targetItems=$view.find('.view-item.item-checked');
      var ev='fm.unchecked';
      $targetItems.removeClass('item-checked');
    }
    afterCheck();
    $targetItems.each(function() {
       $(this).find('.view-item-checkbox').trigger(ev,[fileManager.fileDataList[$(this).index()],fileManager.fileDataList])
    });
  });
  //文件选择之后调用，计算选择情况
  function afterCheck() {
