 ...
 $(function(){
	 ...
	 $("#execIdentitylink_table").on('draw.dt',drawTable).DataTable($.extend({},DATA_TABLES.DEFAULT_OPTION,{
       "sAjaxSource": path+"/execIdentitylinkController/getPageData.do",
       "sServerMethod": "post",
       ...
       "aoColumns": [
    			...
    			{ "sTitle": "nodeId","width":"","mDataProp":"nodeId" }, 
    			{ "sTitle": "userId","width":"","mDataProp":"userId" }, 
    			{ "sTitle": "departId","width":"","mDataProp":"departId" }, 
	...
	function model_add(title,w,h){
		var url = path + "/execIdentitylinkController/skipAddExecIdentitylink.do";
		layer_show(title,url,w,h);
	}

	
	function edit_model(title,id,w,h){
	     var url = path+"/execIdentitylinkController/getDetail.do?id="+id;
		 layer_show(title,url,w,h);
	}
	...
	function datadel(){
       ...
       $("#execIdentitylink_table .checkbox:checked").each(function() {		
		  l.push($(this).val());		
	   });
	...
	function sumbitDel(ids){
		$.ajax({
			    url: path+"/execIdentitylinkController/deleteExecIdentitylink.do",
			    traditional:true,
			    dataType:'json',  
			    data:{
			    	"ids":ids
 	...
 	function goSearch(signal){			   
		 $("#execIdentitylink_table").DataTable().draw(signal);					
	 }
