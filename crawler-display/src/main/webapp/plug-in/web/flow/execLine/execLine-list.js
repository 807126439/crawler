 ...
 $(function(){
	 ...
	 $("#execLine_table").on('draw.dt',drawTable).DataTable($.extend({},DATA_TABLES.DEFAULT_OPTION,{
       "sAjaxSource": path+"/execLineController/getPageData.do",
       "sServerMethod": "post",
	...
	function model_add(title,w,h){
		var url = path + "/execLineController/skipAddExecLine.do";
		layer_show(title,url,w,h);
	}

	
	function edit_model(title,id,w,h){
	     var url = path+"/execLineController/getDetail.do?id="+id;
		 layer_show(title,url,w,h);
	}
	...
	function datadel(){
       ...
       $("#execLine_table .checkbox:checked").each(function() {		
		  l.push($(this).val());		
	   });
	...
	function sumbitDel(ids){
		$.ajax({
			    url: path+"/execLineController/deleteExecLine.do",
			    traditional:true,
			    dataType:'json',  
			    data:{
			    	"ids":ids
 	...
 	function goSearch(signal){			   
		 $("#execLine_table").DataTable().draw(signal);					
	 }
