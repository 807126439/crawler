 ...
 $(function(){
	 ...
	 $("#procLine_table").on('draw.dt',drawTable).DataTable($.extend({},DATA_TABLES.DEFAULT_OPTION,{
       "sAjaxSource": path+"/procLineController/getPageData.do",
       "sServerMethod": "post",
       ...
       "aoColumns": [
    			...
    			{ "sTitle": "lineName","width":"","mDataProp":"lineName" }, 
    			{ "sTitle": "lineType","width":"","mDataProp":"lineType" }, 
    			{ "sTitle": "lineCode","width":"","mDataProp":"lineCode" }, 
    			{ "sTitle": "lineAlt","width":"","mDataProp":"lineAlt" }, 
    			{ "sTitle": "midPos","width":"","mDataProp":"midPos" }, 
    			{ "sTitle": "preNodeId","width":"","mDataProp":"preNodeId" }, 
    			{ "sTitle": "nextNodeId","width":"","mDataProp":"nextNodeId" }, 
	...
	function model_add(title,w,h){
		var url = path + "/procLineController/skipAddProcLine.do";
		layer_show(title,url,w,h);
	}

	
	function edit_model(title,id,w,h){
	     var url = path+"/procLineController/getDetail.do?id="+id;
		 layer_show(title,url,w,h);
	}
	...
	function datadel(){
       ...
       $("#procLine_table .checkbox:checked").each(function() {		
		  l.push($(this).val());		
	   });
	...
	function sumbitDel(ids){
		$.ajax({
			    url: path+"/procLineController/deleteProcLine.do",
			    traditional:true,
			    dataType:'json',  
			    data:{
			    	"ids":ids
 	...
 	function goSearch(signal){			   
		 $("#procLine_table").DataTable().draw(signal);					
	 }
