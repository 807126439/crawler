 ...
 $(function(){
	 ...
	 $("#procNode_table").on('draw.dt',drawTable).DataTable($.extend({},DATA_TABLES.DEFAULT_OPTION,{
       "sAjaxSource": path+"/procNodeController/getPageData.do",
       "sServerMethod": "post",
	...
	function model_add(title,w,h){
		var url = path + "/procNodeController/skipAddProcNode.do";
		layer_show(title,url,w,h);
	}

	
	function edit_model(title,id,w,h){
	     var url = path+"/procNodeController/getDetail.do?id="+id;
		 layer_show(title,url,w,h);
	}
	...
	function datadel(){
       ...
       $("#procNode_table .checkbox:checked").each(function() {		
		  l.push($(this).val());		
	   });
	...
	function sumbitDel(ids){
		$.ajax({
			    url: path+"/procNodeController/deleteProcNode.do",
			    traditional:true,
			    dataType:'json',  
			    data:{
			    	"ids":ids
 	...
 	function goSearch(signal){			   
		 $("#procNode_table").DataTable().draw(signal);					
	 }
