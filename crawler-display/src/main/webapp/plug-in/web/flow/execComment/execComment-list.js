 ...
 $(function(){
	 ...
	 $("#execComment_table").on('draw.dt',drawTable).DataTable($.extend({},DATA_TABLES.DEFAULT_OPTION,{
       "sAjaxSource": path+"/execCommentController/getPageData.do",
       "sServerMethod": "post",
       ...
       "aoColumns": [
    			...
    			{ "sTitle": "content","width":"","mDataProp":"content" }, 
	...
	function model_add(title,w,h){
		var url = path + "/execCommentController/skipAddExecComment.do";
		layer_show(title,url,w,h);
	}

	
	function edit_model(title,id,w,h){
	     var url = path+"/execCommentController/getDetail.do?id="+id;
		 layer_show(title,url,w,h);
	}
	...
	function datadel(){
       ...
       $("#execComment_table .checkbox:checked").each(function() {		
		  l.push($(this).val());		
	   });
	...
	function sumbitDel(ids){
		$.ajax({
			    url: path+"/execCommentController/deleteExecComment.do",
			    traditional:true,
			    dataType:'json',  
			    data:{
			    	"ids":ids
 	...
 	function goSearch(signal){			   
		 $("#execComment_table").DataTable().draw(signal);					
	 }
