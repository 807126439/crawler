...
//初始化区域图的对象
	...
	this.canvasSize=property.size||3;//左侧工具栏对象
	...
	this.$newItem={};//表示新添加的item
	this.$defaultNodeSize=property.nodeSize||[24,24];//表示新添加的item
	var headHeight=0;
	var tmp="";
	
	if(property.haveHead)
		this.$bgDiv.addClass("has-head");
	else
		this.$bgDiv.removeClass("has-head");
	if(property.haveTool)
		this.$bgDiv.addClass("has-tool");
	else
		this.$bgDiv.removeClass("has-tool");

	if(property.haveHead){
		this.$bgDiv.addClass("has-head");
		tmp="<div class='GooFlow_head'>";
		if(property.headLabel){
      tmp+="<div class='flowhead-title-wrap'><label class='flowhead-label' title='"+(property.initLabelText||"newFlow_1")+"' "
        +(GooFlow.prototype.color.main? "style='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"newFlow_1")+"</label><input type='text' style='display:none'/></div>";
		}
	  ...
	  //标题编辑
		this.$head.on("click",'.flowhead-label',{inthis:this},function(e){
			var This=e.data.inthis;
			if (!This.$editable) return
			$(this).next('input').show().focus().val(This.$title||property.initLabelText||"newFlow_1");
			This.$bgDiv.on('click',{inthis:This},saveTitle)
		});
		this.$head.on("keyup",'.flowhead-label+input',{inthis:this},function(e){
			var This=e.data.inthis;
			if (e.keyCode == "13") {
				saveTitle({data:{inthis:This}})
			}
		});
		function saveTitle(e){
			if (e) {
				var This=e.data.inthis
				if (e.target&&($(e.target).is('input')||$(e.target).is('label'))) return
			}
			This.$bgDiv.off('click',saveTitle)
			var $tt=This.$head.find('.flowhead-label+input');
			var tt=$tt.val();
			$tt.hide()
			This.setTitle(tt)
			if(This.onTitleChange!=null)
			This.onTitleChange(tt);
		}
	}
	var toolWidth=0;
	if(property.haveTool){
		this.$bgDiv.append("<div class='GooFlow_tool'><div class='GooFlow_tool_div'></div></div>");
     ...
     +"<a href='javascript:void(0)' type='direct-dashed' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct-dashed'><i class='ico_dash'/></a>"
     +"<a href='javascript:void(0)' type='direct-noArrow' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct-noArrow'><i class='ico_straight'/></a>"
    );
	...
	this.$workArea=$("<div class='GooFlow_work_inner' style='width:"+width*(property.size||3)+"px;height:"+height*(property.size||3)+"px'></div>")
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
			...
			else if(type.indexOf("direct")!==-1||type=="group")return;
			var X,Y;
	  ...
	  this.$workArea.mousemove({inthis:this},function(e){
			if(e.data.inthis.$nowType.indexOf("direct")==-1&&!e.data.inthis.$mpTo.data("p"))	return;
			...
			if(lineStart){
					...
					else if (line.childNodes[1].getAttribute("marker-end"))
	  ...
	  this.$workArea.mouseup({inthis:this},function(e){
	  	var This=e.data.inthis;
			if(This.$nowType.indexOf("direct")==-1&&!This.$mpTo.data("p"))	return;
	  ...
	  //当操作某个结点被双击，触发的方法，返回FALSE可阻止选中事件的发生
	  //格式function(id,e,This)：id是单元的唯一标识ID,e是jq事件，This为flow对象,
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			this.$draw.coordsize = width*this.canvasSize+","+height*this.canvasSize;
			this.$workArea.prepend("<div class='GooFlow_work_vml' style='position:relative;width:"+width*this.canvasSize+"px;height:"+height*this.canvasSize+"px'></div>");
			this.$workArea.children("div")[0].insertBefore(this.$draw,null);
		}
		this.$draw.id = id;
		this.$draw.style.width = width*this.canvasSize + "px";
		this.$draw.style.height = +height*this.canvasSize + "px";
		//绑定连线的点击选中以及双击编辑事件
		var tmpClk=null;
	...
	initGroup:function(width,height){
		this.$group=$("<div class='GooFlow_work_group' style='width:"+width*(this.canvasSize||3)+"px;height:"+height*(this.canvasSize||3)+"px'></div>");//存放背景区域的容器
	...
	initLinePointsChg:function(){
		this.$mpFrom.on("mousedown",{inthis:this},function(e){
			...
			var line=GooFlow.prototype.drawLine("GooFlow_tmp_line",[ps[0],ps[1]],[pe[0],pe[1]],true,This.$lineData[This.$lineOper.data("tid")].type.indexOf('dashed')!==-1,This.$lineData[This.$lineOper.data("tid")].noArrow);
	...
	switchToolBtn:function(type){
      ...
      this.blurItem();
		}else if( this.$nowType=="direct-dashed"){
      this.blurItem();
		}else if( this.$nowType=="direct-noArrow"){
      this.blurItem();
		}
	...
	addNode:function(id,json){
		...
		if (json.type.indexOf("empty")>=0) {
				json.width=0;json.height=0;
				$node=$("<div class='GooFlow_item"+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px;height:0px;width:0px;border-right:none;border-bottom:none'><table style='border-collapse: collapse'><tr><td class='ico'><i class='ico_"+json.type+"'></i><div class='move_title'><span class='move_title-id'>"+id+"</span></div></td></tr></table><div class='eidt_panel' style='display:none'><div class='rs_close'></div></div></div>");
		}else if(json.type.indexOf(" round")<0){
			if(!json.width) json.width=this.$defaultNodeSize[0];
			if(!json.height) json.height=this.$defaultNodeSize[1];
			if(!json.width||json.width<100)json.width=100;
			if(!json.height||json.height<24)json.height=24;
			var customObj;
			if (this.customRenderDom) {
				customObj=this.customRenderDom(id,json);
				content= customObj.content;
			}
			$node=$("<div class='GooFlow_item"+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'>\
												<table cellspacing='0' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'>\
													<tr>\
														<td class='ico'><i class='ico_"+json.type+"'></i><div class='move_title'><span class='move_title-id'>"+id+"</span></div></td>\
														<td class='ct'></td>\
													</tr>\
												</table>\
												<div class='eidt_panel' style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div>\
											</div>");
			$node.find('td.ct').append(content||json.name)
			if (customObj&&customObj.extClass) {
				if (customObj.extClass instanceof Array)
					for(var i=0;i<customObj.extClass.length;i++)
						$node.addClass(customObj.extClass[i])
				else
						$node.addClass(customObj.extClass)
			}
		}else{
			json.width=24;json.height=24;
			$node=$("<div class='GooFlow_item item_round"+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'>\
								<table cellspacing='0'>\
									<tr>\
										<td class='ico'><i class='ico_"+json.type+"'></i><div class='move_title'><span class='move_title-id'>"+id+"</span></div></td>\
									</tr>\
								</table>\
								<div class='eidt_panel' style='display:none'><div class='rs_close'></div></div>\
								<div class='span'></div>\
							</div>");
			$node.find('.span').append(json.name)
		}
		$node.addClass('GooFlow_item_'+json.type.split(' ').join('_'))
		this.$nodeDom[id]=$node;
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			...
			if(e.button==2)return false;
			var This=e.data.inthis;
			if(This.$nowType.indexOf("direct")!==-1)	return;
		...
		this.$workArea.delegate(".GooFlow_item","mouseenter",{inthis:this},function(e){
			if(e.data.inthis.$nowType.indexOf("direct")==-1&&!document.getElementById("GooFlow_tmp_line"))	return;
			$(this).addClass("item_mark").addClass("crosshair");
		});
		...
		this.$workArea.delegate(".GooFlow_item","mousedown",{inthis:this},function(e){
			...
			var line=GooFlow.prototype.drawLine("GooFlow_tmp_line",[X,Y],[X,Y],true,This.$nowType.indexOf("dashed")!==-1,This.$nowType.indexOf("noArrow")!==-1);
			This.$draw.appendChild(line);
		});
		//绑定连线时确定结束点
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
				var info={from:lineStart.id,to:this.id,name:"",};
				if (This.$nowType.indexOf("dashed")!==-1) {
					info.dashed=true;
				}
				if (This.$nowType.indexOf("noArrow")!==-1) {
					info.noArrow=true;
				}
				This.addLine(This.$id+"_line_"+This.$max,info);
				This.$max++;
			}
			else{
				if(lineStart){
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			var t=getElCoordinate(This.$workArea[0]);
			if(This.onNodeDblclick!=null&&!This.onNodeDblclick(id,e,This))	return false;
		...
		this.$workArea.delegate(".rs_close","click",{inthis:this},function(e){
			if(!e)e=window.event;
			e.data.inthis.delNode(e.data.inthis.$focus,true);
			return false;
		});
		//绑定结点的RESIZE功能
	...
	blurItem:function(){
		if(this.$focus!=""){
			...
			if(jq.prop("tagName")=="DIV"){
				...
				jq.removeClass("item_focus").children(".eidt_panel").css("display","none");
			}
			else{
				...
				if(GooFlow.prototype.useSVG!=""){
					if(!this.$lineData[this.$focus].marked){
						...
						if (!this.$lineData[this.$focus].noArrow)
	...
	focusItem:function(id,bool){
		...
		if(jq.prop("tagName")=="DIV"){
			...
			if(this.$editable)jq.children(".eidt_panel").css("display","block");
	...
	resizeNode:function(id,width,height){
		...
		if(this.$nodeData[id].type=="start_round"||this.$nodeData[id].type=="end_round")return;
		if(this.$undoStack){
	...
	//删除结点
	delNode:function(id,sure){
		if(!this.$nodeData[id])	return;
		if(this.onItemDel!=null&&!this.onItemDel(id,"node"))	return;
		if(sure&&!confirm("确定要删除该节点和其连线吗?")){
	      return;
		}
		this.blurItem();
		//先删除可能的连线
		...
		if(this.$editable){
			//在回退新增操作时,如果节点为本次编辑时新加入的节点,这些节点的删除不用加入到$deletedItem中
			if(!this.$newItem[id])
	...
	setTitle:function(text){
		this.$title=text;
		if(this.$head)	this.$head.find(".flowhead-label").attr("title",text).text(text);
	},
	//载入一组数据
	...
	//清空工作区及已载入的数据
	clearData:function(ignoreDelItem){
...
///////////以下为有关画线的方法
	//绘制一条箭头线，并返回线的DOM
	drawLine:function(id,sp,ep,mark,dash,noArrow){
		var line;
		if(GooFlow.prototype.useSVG!=""){
	...
	//画一条只有两个中点的折线
	drawPoly:function(id,sp,m1,m2,ep,mark,dash,noArrow){
		var poly,strPath;
		if(GooFlow.prototype.useSVG!=""){
	...
	addLineDom:function(id,lineData){
		...
		var dash=false
		if(lineData.dashed&&lineData.dashed===true)
			dash=true;
		
		if(lineData.type=="sl")
			this.$lineDom[id]=GooFlow.prototype.drawLine(id,res.start,res.end,lineData.marked,dash,lineData.noArrow);
		else
			this.$lineDom[id]=GooFlow.prototype.drawPoly(id,res.start,res.m1,res.m2,res.end,lineData.marked,dash,lineData.noArrow);
	...
	addLine:function(id,json){
		...
		if (json.noArrow) this.$lineData[id].noArrow=json.noArrow;
		if (json.dashed) this.$lineData[id].dashed=json.dashed;
	...
	resetLines:function(id,node){
		for(var i in this.$lineData){
		  ...
		  if(this.$lineData[i].dashed&&this.$lineData[i].dashed===true)
		  	dash=true;

		  if(this.$lineData[i].type=="sl"){
		  	this.$lineDom[i]=GooFlow.prototype.drawLine(i,res.start,res.end,this.$lineData[i].marked,dash,this.$lineData[i].noArrow);
		  }
		  else{
			this.$lineDom[i]=GooFlow.prototype.drawPoly(i,res.start,res.m1,res.m2,res.end,this.$lineData[i].marked,dash,this.$lineData[i].noArrow);
		  }
	...
	setLineType:function(id,newType,M){
		...
		//如果是变回直线
		  ...
		  var dash=false;
		  if(this.$lineData[id].dashed&&this.$lineData[id].dashed===true)
		  	dash=true;
		  this.$lineDom[id]=GooFlow.prototype.drawLine(id,res.start,res.end,this.$lineData[id].marked||this.$focus==id,dash,this.$lineData[id].noArrow);
		  this.$draw.appendChild(this.$lineDom[id]);
		  if(GooFlow.prototype.useSVG==""){
	...
	setLineM:function(id,M,noStack){
	  ...
	  if(this.$lineData[id].dashed&&this.$lineData[id].dashed===true)
		...
		this.$draw.removeChild(this.$lineDom[id]);
		this.$lineDom[id]=GooFlow.prototype.drawPoly(id,ps.start,ps.m1,ps.m2,ps.end,this.$lineData[id].marked||this.$focus==id,dash,this.$lineData[id].noArrow);
		this.$draw.appendChild(this.$lineDom[id]);
		if(GooFlow.prototype.useSVG==""){
			this.$lineDom[id].childNodes[1].innerHTML=this.$lineData[id].name;
	...
	//删除转换线
	delLine:function(id,sure){
		if(!this.$lineData[id])	return;
		if(this.onItemDel!=null&&!this.onItemDel(id,"node"))	return;
		if(sure&&!confirm("确定要删除该连线吗?")){
	      return;
		}
    this.blurItem();
		if(this.$undoStack){
		...
		--this.$lineCount;
		if(this.$editable){
			//在回退新增操作时,如果连线为本次编辑时新加入的连线,这些连线的删除不用加入到$deletedItem中
			if(!this.$newItem[id])
	...
	delArea:function(id){
		...
		--this.$areaCount;
		if(this.$editable){
			//在回退新增操作时,如果区域为本次编辑时新加入的区域,这些区域的删除不用加入到$deletedItem中
			if(!this.$newItem[id])
	...
	reinitSize:function(width,height){
		...
		this.$workArea.css({height:h*this.canvasSize+"px",width:w*this.canvasSize+"px"});
		if(GooFlow.prototype.useSVG==""){
			this.$draw.coordsize = w*this.canvasSize+","+h*this.canvasSize;
		}
		this.$draw.style.width = w*this.canvasSize + "px";
		this.$draw.style.height = +h*this.canvasSize + "px";
		if(this.$group==null){
			this.$group.css({height:h*this.canvasSize+"px",width:w*this.canvasSize+"px"});
		}
	},
	//更新数据状态
	saveDataState:function(){
		for(var k1 in this.$nodeData){
			if(typeof this.$nodeData[k1].alt!=='undefined'){
				delete this.$nodeData[k1]["alt"];
			}
		}
		for(var k2 in this.$lineData){
			if(typeof this.$lineData[k2].alt!=='undefined'){
				delete this.$lineData[k2]["alt"];
			}
		}
		for(var k3 in this.$areaData){
			if(typeof this.$areaData[k3].alt!=='undefined'){
				delete this.$areaData[k3]["alt"];
			}
		}
		this.$deletedItem={};
		this.$newItem={};
