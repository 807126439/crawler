 ...
 <tr class="odd">
    ...
    <td>
    	 <c:choose>
		    ...
		    <c:otherwise>
		        <input type="text" class="input-text "  id="unit" name="unit"  datatype="*2-100" nullmsg="请输入单位名称！" value="${userItem.unit}"  >
		    </c:otherwise> 
	  	</c:choose> 
    
    </td>
    <td width="130">单位性质</td>
    <td>
    	 <c:choose>
		    <c:when test="${isView eq true}">
		    	 ${userItem.unitNature}
  ...
  <tr class="odd">
    ...
    <td>    	
    	 <c:choose>
		    ...
		    <c:otherwise>
		        <input type="text" class="input-text" id="unitLevel" name="unitLevel" datatype="*1-50" nullmsg="请输入单位级别！" value="${userItem.unitLevel}">
		    </c:otherwise> 
	  	</c:choose>
    </td>
    <td>法人代表</td>
